              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    FilledButton(
                      onPressed: busy ? null : (sent ? _verify : _sendOtp),
                    ...
                    const Text(
                      'حساب الصنايعي يجب أن يكون مفعّلاً من الإدارة قبل استقبال الطلبات.',
...
class _ProHomeState extends State<ProHome> {
  ...
  Future<void> _load() async {
    ...
    try {
      final rows = await client
          .from('requests')
          .select('id,title,description,address_text,pricing_mode,status,scheduled_at')
          .inFilter('status', ['published', 'offers_received'])
          .order('created_at', ascending: false);

      final jobs = await client
          .from('jobs')
          .select('id,request_id,customer_id,agreed_amount,pricing_mode,arrived_at,started_at,completed_at,requests(title,address_text,status)')
          .eq('professional_id', client.auth.currentUser!.id)
          .order('created_at', ascending: false)
          .limit(1);

      requests = List<Map<String, dynamic>>.from(rows);
      activeJob = jobs.isEmpty ? null : Map<String, dynamic>.from(jobs.first);
      jobMessage = activeJob == null
          ? 'لا يوجد عمل نشط'
          : 'عمل فعلي مرتبط بطلب ${activeJob!['request_id']}';
    } catch (e) {
      jobMessage = e.toString();
    }
  ...
  Future<void> _submitOffer(Map<String, dynamic> request) async {
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text('${modeLabel(request['pricing_mode'] ?? '')} • ${request['address_text'] ?? 'الموقع يحدد لاحقاً'}'),
                  const SizedBox(height: 14),
                  Text(request['description'] ?? 'بدون وصف إضافي.'),
                  const SizedBox(height: 18),
                  TextField(
                    controller: price,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(
                      labelText: 'سعرك بالدينار',
                      border: OutlineInputBorder(),
                    ),
                  ),
                  const SizedBox(height: 12),
                    ...
                    onPressed: () async {
                      ...
                      if (value == null || value <= 0) {
                        ScaffoldMessenger.of(sheetContext).showSnackBar(
                          const SnackBar(content: Text('اكتب سعراً صحيحاً.')),
                        );
                        return;
                      }
        ...
        Container(
          ...
          child: const Column(
            ...
            children: [
              ...
              SizedBox(height: 8),
              Text(
                'اختار الطلب المناسب، أرسل سعرك، وبعد قبول العرض ينتقل لشغلك تلقائياً.',
        ...
        ...requests.map(
          (request) => Card(
            child: ListTile(
              ...
              subtitle: Text(
                '${modeLabel(request['pricing_mode'] ?? '')}\n${request['address_text'] ?? 'الموقع يحدد لاحقاً'}',
              ),
      ...
      children: [
        ...
        Card(
          child: Padding(
            padding: const EdgeInsets.all(18),
            child: job == null
                ? const Text('لسه ما عندك شغل مقبول.')
                : Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(
                        (job['requests'] as Map?)?['title'] ?? 'عمل نشط',
                        style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900),
                      ),
                      const SizedBox(height: 6),
                      Text('${job['agreed_amount']} د.أ • ${modeLabel(job['pricing_mode'] ?? '')}'),
                      const SizedBox(height: 6),
                      Text((job['requests'] as Map?)?['address_text'] ?? 'الموقع يحدد لاحقاً'),
                      ...
                      Wrap(
                        ...
                        children: [
                          FilledButton.icon(
                            onPressed: () => _setJobStatus('en_route'),
                            icon: const Icon(Icons.navigation),
                            label: const Text('بالطريق'),
                          ),
                          FilledButton.icon(
                            onPressed: () => _setJobStatus('arrived'),
  ...
  Future<Map<String, dynamic>> _myProfile() async {
    ...
    final pro = await client
        ...
        .select('skills,service_areas,verification_status,rating,completed_jobs,is_available')
        .eq('user_id', userId)
        .maybeSingle();

    final result = Map<String, dynamic>.from(profile);
    if (pro != null) {
      result.addAll(Map<String, dynamic>.from(pro));
    }
    return result;
  }
  ...
  Widget _profile() {
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        final areas = profile['service_areas'];
        final areaText = areas is List ? areas.join('، ') : (areas ?? 'عمّان');
          ...
          children: [
            ...
            Card(
              child: ListTile(
                ...
                title: Text(profile['full_name'] ?? 'صنايعي'),
                subtitle: Text('${profile['phone'] ?? ''} • ${profile['city'] ?? 'عمّان'}'),
              ),
            ),
            Card(
              child: Column(
                children: [
                  ListTile(
                    ...
                    title: Text('الحالة: ${profile['verification_status'] ?? 'pending'}'),
                  ),
                  ListTile(
                    leading: const Icon(Icons.star),
                    title: Text('التقييم: ${profile['rating'] ?? 0}'),
                  ),
                  ListTile(
                    leading: const Icon(Icons.task_alt),
                    title: Text('طلبات مكتملة: ${profile['completed_jobs'] ?? 0}'),
                  ),
                  ListTile(
                    leading: const Icon(Icons.location_on_outlined),
                    title: Text('مناطق العمل: $areaText'),
                  ),
                ],
              ),
            ),
